...
Public Class PagosAnula
    ...
    Private Sub btnBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBuscar.Click
        ...
        Try
            ...
            sql &= ", (select top 1 FechaMovimiento from FacturasMovimientos as m where m.NumeroFactura = f.NumeroFactura and m.IdProducto = f.IdProducto and m.IdPoliza = f.IdPoliza and m.idramo = f.idramo and m.TipoMovimiento = 'PAGO' order by IdMovimiento desc) as FechaPagoFact"
            sql &= " , isnull((select sum(m.PrimaNeta) * -1 from FacturasMovimientos as m where m.NumeroFactura = f.NumeroFactura and m.IdProducto = f.IdProducto and m.IdPoliza = f.IdPoliza and m.idramo = f.idramo and m.TipoMovimiento = 'PAGO' ),0) as Prima"
            sql &= " , isnull((select sum(m.GastosEmision) * -1 from FacturasMovimientos as m where m.NumeroFactura = f.NumeroFactura and m.IdProducto = f.IdProducto and m.IdPoliza = f.IdPoliza and m.idramo = f.idramo and m.TipoMovimiento = 'PAGO'),0) as GastosEmision"
            sql &= " , isnull((select sum(m.GastosFracciona) * -1 from FacturasMovimientos as m where m.NumeroFactura = f.NumeroFactura and m.IdProducto = f.IdProducto and m.IdPoliza = f.IdPoliza and m.idramo = f.idramo and m.TipoMovimiento = 'PAGO'),0) as GastosFracciona"
            sql &= " , isnull((select sum(m.GastosBomberos) * -1 from FacturasMovimientos as m where m.NumeroFactura = f.NumeroFactura and m.IdProducto = f.IdProducto and m.IdPoliza = f.IdPoliza and m.idramo = f.idramo and m.TipoMovimiento = 'PAGO'),0) as GastosBomberos"
            sql &= " , isnull((select sum(m.Gastos) * -1 from FacturasMovimientos as m where m.NumeroFactura = f.NumeroFactura and m.IdProducto = f.IdProducto and m.IdPoliza = f.IdPoliza and m.idramo = f.idramo and m.TipoMovimiento = 'PAGO'),0) as Gastos"
            sql &= " , isnull((select sum(m.Impuestos) * -1 from FacturasMovimientos as m where m.NumeroFactura = f.NumeroFactura and m.IdProducto = f.IdProducto and m.IdPoliza = f.IdPoliza and m.idramo = f.idramo and m.TipoMovimiento = 'PAGO'),0) as Iva"
            sql &= " , isnull((select sum(m.RetencionImpuestos) * -1 from FacturasMovimientos as m where m.NumeroFactura = f.NumeroFactura and m.IdProducto = f.IdProducto and m.IdPoliza = f.IdPoliza and m.idramo = f.idramo and m.TipoMovimiento = 'PAGO'),0) as RetencionImpuestos"
            sql &= " , isnull((select sum(m.Total) * -1 from FacturasMovimientos as m where m.NumeroFactura = f.NumeroFactura and m.IdProducto = f.IdProducto and m.IdPoliza = f.IdPoliza and m.idramo = f.idramo and m.TipoMovimiento = 'PAGO'),0) as Total"
            sql &= " , isnull((select sum(m.Comision) * -1 from FacturasMovimientos as m where m.NumeroFactura = f.NumeroFactura and m.IdProducto = f.IdProducto and m.IdPoliza = f.IdPoliza and m.idramo = f.idramo and m.TipoMovimiento = 'PAGO'),0) as Comision"
            ...
            sql &= "  and (select sum(m.Total) * -1 from FacturasMovimientos as m where m.NumeroFactura = f.NumeroFactura and m.IdProducto = f.IdProducto and m.IdPoliza = f.IdPoliza and m.idramo = f.idramo and m.TipoMovimiento = 'PAGO') > 0 "
            ...
            If Facturas.Tables(0).Rows.Count = 0 Then
                MsgBox("No se encontraron pagos para anular con las condiciones dadas.", MsgBoxStyle.Information, "AVISO...")
            End If
    ...
    Private Sub btnGuardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGuardar.Click
        ...
        If txtFechaAnula.Text = "" Then
            MsgBox("Debe especificar la fecha efectiva de Anulación de Pago.", MsgBoxStyle.Exclamation, "AVISO...")
            Exit Sub
        End If
        If txtMotivo.Text = "" Then
            MsgBox("Debe especificar el motivo de Anulación de Pago.", MsgBoxStyle.Exclamation, "AVISO...")
            Exit Sub
        End If
