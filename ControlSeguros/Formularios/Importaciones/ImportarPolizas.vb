...
Public Class ImportarPolizas
    ...
    Private Sub btnBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBuscar.Click
        Try
            ...
            Hoja.Cells(Fila, 20).Value = "NUMERO VIGENCIA"
            Hoja.CELLS(Fila, 21).VALUE = "PORCE COMISION"

            Hoja.Cells(Fila, 22).Value = "FORMA DE PAGO"
            Hoja.Cells(Fila, 23).Value = "ESTADO POLIZA"
            Hoja.Cells(Fila, 24).Value = "COD CANCELACION"
            Hoja.Cells(Fila, 25).Value = "VENDEDOR"
            Hoja.Cells(Fila, 26).Value = "FECHA ENVIO ANEXO"
            Hoja.Cells(Fila, 27).Value = "FECHA SOL RENOVACION"
            Hoja.Cells(Fila, 28).Value = "FECHA DE RECEPCION"
    ...
    Public Function DeterminarColumna(ByVal NombreColumna As String) As String
        ...
        Try
            ...
            Select Case NombreColumna
                ...
                Case "NUMEROVIGENCIA"
                    Res = "G|NumVigencia|"
                Case "PORCECOMISION", "COMISION"
                    Res = "G|PorceComision|"


                Case "FORMADEPAGO", "FORMAPAGO"
                    Res = "G|IdFormaPago|"
                Case "ESTADOPOLIZA", "ESTADO"
                    Res = "G|EstadoPoliza|"
                Case "CODCANCELACION", "MOTIVOCANCELACION"
                    Res = "G|MotivoCancelacion|"
                Case "VENDEDOR"
                    Res = "G|IdVendedor|"
                Case "FECHAENVIOANEXO", "FECHAANEXO"
                    Res = "G|FechaEnvioAnexo|"
                Case "FECHASOLRENOVACION", "FECHARENOVACION"
                    Res = "G|FechaSolicitudRenovacion|"
                Case "FECHADERECEPCION", "FECHARECEPCION"
                    Res = "G|FechaRecepcion|"
    ...
    Private Function CrearPoliza(ByRef Hoja As Object, ByVal Fila As Integer, ByVal Revision As Boolean) As String
        ...
        If ExisteCampo("IdFormaPago") = True Then
            IdFormaPago = ObtenerDatoArchivo(Hoja, Fila, "IdFormaPago").Trim
        End If

        If ExisteCampo("EstadoPoliza") = True Then
            EstadoPoliza = ObtenerDatoArchivo(Hoja, Fila, "EstadoPoliza").Trim
        End If

        If ExisteCampo("MotivoCancelacion") = True Then
            MotivoCancelacion = ObtenerDatoArchivo(Hoja, Fila, "MotivoCancelacion").Trim
        End If

        Dim IdVendedor As String = ""
        If ExisteCampo("IdVendedor") = True Then
            IdVendedor = ObtenerDatoArchivo(Hoja, Fila, "IdVendedor").Trim
            'validaremos que sea vendedor existente y valido
            If IdVendedor <> "" Then IdVendedor = DeterminarVendedor(IdVendedor)
        End If

        Dim FechaEnvioAnexo As String = ""
        If ExisteCampo("FechaEnvioAnexo") = True Then
            FechaEnvioAnexo = ObtenerDatoArchivo(Hoja, Fila, "FechaEnvioAnexo").Trim
        End If

        Dim FechaSolicitudRenovacion As String = ""
        If ExisteCampo("FechaSolicitudRenovacion") = True Then
            FechaSolicitudRenovacion = ObtenerDatoArchivo(Hoja, Fila, "FechaSolicitudRenovacion").Trim
        End If


        Dim NumVigencia As Integer = 1
        If ExisteCampo("NumVigencia") = True Then
            If ObtenerDatoArchivo(Hoja, Fila, "NumVigencia").Trim = "" Then
                NumVigencia = 1
            Else
                NumVigencia = CInt(ObtenerDatoArchivo(Hoja, Fila, "NumVigencia").Trim)
            End If

        End If

        Dim FechaRecepcion As String = ""
        If ExisteCampo("FechaRecepcion") = True Then
            FechaRecepcion = ObtenerDatoArchivo(Hoja, Fila, "FechaRecepcion").Trim
        End If
        Dim PorceComision As Double = 0
        If ExisteCampo("PorceComision") = True Then
            PorceComision = Val(ObtenerDatoArchivo(Hoja, Fila, "PorceComision").Trim)
        End If
            ...
            If ExistePol > 0 Then
                Try
                    ...
                    If Res = "" And Revision = False Then
                        ...
                        sql &= ", NumVigencia = " & dbInt(NumVigencia)
                        sql &= ", IdFormaPago = " & dbStr(IdFormaPago, 50)
                        sql &= ", FechaEnvioAnexo = " & dbFec(FechaEnvioAnexo)
                        sql &= ", FechaSolicitudRenovacion = " & dbFec(FechaSolicitudRenovacion)
                        sql &= ", FechaRecepcion = " & dbFec(FechaRecepcion)
                        ...
                        'ingresamos al vendedor
                        If IdVendedor <> "" Then
                            StiGlobalConn.SQLExecute("delete from PolizasVendedores where IdPoliza = " & dbStr(IdPoliza, 25) & " and IdProducto = " & dbStr(IdProducto, 25))

                            Dim IdTipoVendedor As String = ""
                            Try
                                IdTipoVendedor = StiGlobalConn.ObtenerDataset("select IdTipoVendedor from Vendedores where IdVendedor = " & dbFec(IdVendedor)).Tables(0).Rows(0).Item(0)
                            Catch ex As Exception
                                IdTipoVendedor = ""
                            End Try

                            If IdTipoVendedor <> "" Then
                                sql = " insert into PolizasVendedores "
                                sql &= " (IdPoliza, IdProducto, Orden, IdVendedor, IdTipoVendedor, FechaEfectiva) values("
                                sql &= dbStr(IdPoliza, 25) & C & dbStr(IdProducto, 25) & C & dbInt(1) & C & dbStr(IdVendedor, 25) & C & dbStr(IdTipoVendedor, 25) & C & dbFec(VigenciaDesde) & ") "
                                StiGlobalConn.SQLExecute(sql)
                            End If
            ...
            Else
                Try
                    ...
                    If Res = "" And Revision = False Then
                        ...
                        sql &= " VigenciaHasta, EstadoPoliza, NumVigencia, FechaCancelacion, MotivoCancelacion,"
                        sql &= "IdFormaPago, FechaEnvioAnexo, FechaSolicitudRenovacion, FechaRecepcion, PorceComision ) values("
                        sql &= dbStr(IdPoliza, 25) & C & dbStr(IdProducto, 25) & C & dbStr(IdCliente, 25) & C & dbStr(NombreCliente, 150) & C & dbFec(VigenciaDesde) & C
                        sql &= dbFec(VigenciaHasta) & C & dbStr(EstadoPoliza) & C & dbInt(NumVigencia) & C & dbFec(FechaCancelacion) & C & dbStr(MotivoCancelacion, 50) & C
                        sql &= dbStr(IdFormaPago, 50) & C & dbFec(FechaEnvioAnexo) & C & dbFec(FechaSolicitudRenovacion) & C & dbFec(FechaRecepcion) & C & dbNum(PorceComision) & ")"
                        StiGlobalConn.SQLExecute(sql)
                        ...
                        If IdVendedor <> "" Then
                            ...
                            Dim IdTipoVendedor As String = ""
                            Try
                                IdTipoVendedor = StiGlobalConn.ObtenerDataset("select IdTipoVendedor from Vendedores where IdVendedor = " & dbStr(IdVendedor)).Tables(0).Rows(0).Item(0)
                            Catch ex As Exception
                                IdTipoVendedor = ""
                            End Try
    ...
    Private Function DeterminarVendedor(ByVal parVendedor) As String
        Dim res As String = ""
        Try
            'intentamos con Id
            Try
                res = StiGlobalConn.ObtenerDataset("select IdVendedor from Vendedores where IdVendedor = " & dbStr(parVendedor)).Tables(0).Rows(0).Item(0)
            Catch ex As Exception
                res = ""
            End Try

            'intentamos con nombre completo
            If res = "" Then
                Try
                    res = StiGlobalConn.ObtenerDataset("select IdVendedor from Vendedores where NombreVendedor = " & dbStr(parVendedor)).Tables(0).Rows(0).Item(0)
                Catch ex As Exception
                    res = ""
                End Try
            End If

            'intentamos con nombre similar
            If res = "" Then
                Try
                    res = StiGlobalConn.ObtenerDataset("select IdVendedor from Vendedores where NombreVendedor like " & dbStr("%" & parVendedor & "%")).Tables(0).Rows(0).Item(0)
                Catch ex As Exception
                    res = ""
                End Try
            End If
            ...
            'finalmente por nombres parciales
            If res = "" Then
                Dim NumP As Integer = CuentaCar(parVendedor.Trim, " ") + 1
                For NP As Integer = NumP - 1 To 1 Step -1
                    res = stiSplit(parVendedor, "G0 " & NP)
                    Try
                        res = StiGlobalConn.ObtenerDataset("select IdVendedor from Vendedores where NombreVendedor like " & dbStr("%" & res & "%")).Tables(0).Rows(0).Item(0)
                        Exit For
                    Catch ex As Exception
                        res = ""
                    End Try
